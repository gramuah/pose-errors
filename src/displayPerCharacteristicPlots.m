function [resutclass,f] = displayPerCharacteristicPlots(results_all, error_type)
...
drawline = false;
makeMultiCategoryPlot(f, results_all, 'tmp', [results_all(o).name ': Part Visibility Effect'], 1, tickstr, drawline);
axisval = axis;
n=0;
for o = 1:numel(results_all)
  if isfield(results_all(o).pose, 'part')
...
drawline = false;
makeMultiCategoryPlot(f, results_all, 'tmp', [results_all(o).name ': Visible Side Influence'], 1, tickstr, drawline);
axisval = axis;
n=0;
for o = 1:numel(results_all)
  if isfield(results_all(o).pose, 'side')
