...
for o = 1:numel(result)
  ...
  for k = 1:length(result.det.conf)
    ...
    flag = 0;
    if(error_azimuth <= 15)
        text(bbox(1)+3, bbox(2)+85, sprintf('%d',view(1)), 'backgroundcolor', [1 1 1], 'color', [0 0 1], 'fontsize', 14, 'FontWeight', 'bold');
        plot(xVec,yVec, 'b', 'linewidth', 8);
        outDir = [result.resultdir,'/', result.name, '/qualitative/good/', num2str(c), '_', rec(gt.rnum(si(k))).filename, '.pdf'];
        saveas(h,outDir);
        flag = 1;
    end
    if(error_azimuth >165)
        text(bbox(1)+3, bbox(2)+85, sprintf('%d',view(1)), 'backgroundcolor', [1 0 0], 'fontsize', 14, 'FontWeight', 'bold');
        plot(xVec,yVec, 'r', 'linewidth', 8);
        outDir = [result.resultdir,'/', result.name, '/qualitative/bad_opp/', num2str(c), '_', rec(gt.rnum(si(k))).filename, '.pdf'];
        saveas(h,outDir);
        flag = 1;
    end
    if(error_azimuth >15 && error_azimuth < 30)
        text(bbox(1)+3, bbox(2)+85, sprintf('%d',view(1)), 'backgroundcolor', [1 0 0], 'fontsize', 14, 'FontWeight', 'bold');
        plot(xVec,yVec, 'r', 'linewidth', 8);
        outDir = [result.resultdir,'/', result.name, '/qualitative/bad_nearby/', num2str(c), '_', rec(gt.rnum(si(k))).filename, '.pdf'];
        saveas(h,outDir);
        flag = 1;
    end
    if(flag == 0)
        text(bbox(1)+3, bbox(2)+85, sprintf('%d',view(1)), 'backgroundcolor', [1 0 0], 'fontsize', 14, 'FontWeight', 'bold');
        plot(xVec,yVec, 'r', 'linewidth', 8);
        outDir = [result.resultdir,'/', result.name, '/qualitative/bad_other/', num2str(c), '_', rec(gt.rnum(si(k))).filename, '.pdf'];
        saveas(h,outDir);
    end
    c=c+1;
  end
